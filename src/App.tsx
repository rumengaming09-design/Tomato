...
const LANGUAGES = {
  BG: {
    ...
    vipAtmosphere: [
      ...
      },
      {
        ...
        image: "https://i.postimg.cc/HLcb26ks/IMG-0693(1).jpg"
      }
    ],
  ...
  EN: {
    ...
    gallery: [
      { url: "https://i.postimg.cc/Gt7p61Mc/IMG-0692(1).jpg", title: "Masterly Preparation" },
      { url: "https://i.postimg.cc/L5CsGMys/IMG-0694(1).jpg", title: "New Moments" },
      { url: "https://i.postimg.cc/6qjQFNHJ/viber-izobrazenie-2026-05-19-19-25-05-010.jpg", title: "Tomato Interior" },
...
export default function App() {
  ...
  const cleanupExpiredReservations = useCallback(async (reservations: any[]) => {
    ...
    const toDelete = reservations.filter(res => {
      try {
        ...
        // Deleting 30 minutes after arrival as requested (5:00 PM -> 5:30 PM)
  ...
  const getTimeSlotsForDate = useCallback((dateString: string) => {
    ...
    if (day >= 1 && day <= 5) { // Mon-Fri: 8 AM to 11 PM
      const slots = [];
      for (let h = 8; h <= 22; h++) {
  ...
  useEffect(() => {
    // Show capacity for current slots in all halls
  ...
  useEffect(() => {
    // Initial booking time setup based on date
  ...
  async function handleBooking(e: any) {
    ...
    // Guard: Check if time is selected (will be empty on Sundays)
    if (!bookingTime) {
    ...
    if (hour >= 19 || bookingTime === "00:00") {
      // Whole night: from bookingTime onwards
      affectedSlots = daySlots.filter(s => s >= bookingTime);
    } else {
      // 2 hours ahead
      // We use a dummy date to handle time calculations safely
      const [h, m] = bookingTime.split(':').map(Number);
      const totalMinutesStart = h * 60 + m;
      const totalMinutesEnd = totalMinutesStart + 120; // 2 hours
    ...
    try {
      await runTransaction(db, async (transaction) => {
        ...
        // Check capacity for EACH affected slot
        ...
        // Update capacity for ALL affected slots
      ...
      // Update live spots if booking was for today
      const dateStr = new Date().toISOString().split('T')[0];
    ...
    <div ref={containerRef} className="relative bg-jazz-black selection:bg-jazz-gold selection:text-jazz-black min-h-screen">
      ...
      <nav className={`fixed top-0 left-0 w-full z-[80] flex justify-between items-center px-6 md:px-16 py-6 transition-all duration-500 ${scrolled ? 'backdrop-blur-md border-b border-white/5 bg-jazz-black/50' : 'bg-transparent border-transparent'}`}>
        ...
        <div className="flex items-center gap-4 md:gap-10">
          {/* Language Switcher moved next to reservation button */}
          ...
          {/* Global Menu Toggle (Two Stripes) */}
          <button 
